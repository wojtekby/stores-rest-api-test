    ...
    def test_delete_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEqual(response.status_code, 200)
                self.assertDictEqual({'message': 'Item deleted'}, response.json)
    ...
    def test_create_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'message': "An item with name 'test' already exists."}, resp.json)
    ...
    def test_put_item_list(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'items': [{'name': 'test', 'price': 5.99}]}, resp.json)
                self.assertEqual(resp.status_code, 200)
